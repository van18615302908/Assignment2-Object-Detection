# Assignment 2: Object Detection for Autonomous Driving

In this assignment, you will develop an object detection system for autonomous driving. You are required to **implement a simple detection network** called *AD-Detector (Naïve Version)* with ResNet as Backbone and YOLO as detection head. Through this assignment, you are expected to learn about designing and optimizing an object detection network.

This codebase is only for this course. Please **don't upload your answers or this codebase** to **any** public platforms (e.g., Github) before permitted. This assignment is adapted with permission from HKU DASC7606 (Zhixuan Liang & Haibao Yu), All rights reserved.
...
+ You must submit the test data's model outputs with your completed codes, the test data annotations will be released before deadline. You should utilize hyperparameter tuning and other widely-used techniques to improve detection performance.
...
### 2.0 Dataset and Codebase Preparation
The dataset is available [here](http://10.177.60.243:9070/#/?code=SCHD2). The dataset is composed of training and validation images with their corresponding annotations. Please refer to [src/data/README.md](./src/data/README.md) for the dataset strcuture explanation. 

Unzip and place the downloaded data in your Assignment 2 folder, the codebase is organized as followings.

```
├── dataset
│   ├── annotations					# annotations in json file
...
└── src
    ...
    │   └── ADdetector.py   # object detection network
    ├── utils
    │   ├── loss.py
    │   └── util.py
...
### 2.1 Build and Train Your Detection Model
...
You should fill in three blanks in file [src/model/ADdetector.py](src/model/ADdetector.py).

#### Task 4: Filling in the Object Detection Loss Function
You should fill in three blanks to complete object detection loss in file [src/utils/loss.py](src/utils/loss.py). The object detection loss includes five terms as following (which is Equation (3) in [YOLO v1](https://arxiv.org/abs/1506.02640)). You are required to complete two terms of them.
...
```bash
python src/train.py --output_dir 'checkpoints'
```
...
```bash
python src/predict.py --image_path "./dataset/val/xxxx.jpg" --vis_dir "./vis_results"
```
...
You should train your model on the **training** set, and valid your model on the **val** set, and you should generate and submit the outputs of the test set with your trained detection model.
test data will be released one day before deadline.
...
```bash
python src/for_submit.py --split 'test' --output_file "./result.pkl" --model_path <your best ckpt file> --image_size <your image size>
```

#### Task 8: Improving the Detection Performance
In this task, you should try different techniques to improve the detection performance, such as using grid search for the proper hyperparameters, modifying data preprocessing methods, optimizing the loss for each class, changing the batch size, replacing the optimizer and so on. You should generate the prediction outputs with the trained detection model on the test data. The output organization should follow the form in the next Section.
...
```
└── uid_name_assignment_2 # Your submission file name.  
...
### 3.2 Score Scheme
The final score of the assignment is composed of four parts: (1) the codebase you complete (40%); (2) the detection performance (40%); (3) the experiment analysis report (20%).

1. For the completed codebase (40%): The mark will be given mainly based on completeness under the requirements.

2. For the performance part (40%): TA will rerun your code on another test dataset (same distribution as training set and verification set) and give your marks based on the mAP of your model.
...
**Computing Resource Reference:** Training on a single NVIDIA RTX 3090  GPU with a batch size of 16 (use less than 12GB VRAM) takes less than ~2 minutes per epoch.

## 5. Important Date

- Assignment 2 Release: Nov 17 (Mon)
- Test Data Release: Nov 28 (Fri)
- Submission Deadline: Nov 30 (Sun)
